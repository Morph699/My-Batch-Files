...
chcp 65001 >nul
CD /d "%~dp0"
TITLE Morphs Win11 Installer Menu

:MENU
mode con:cols=85 lines=55
cls
color 1F
cls
...
echo.
echo     This menu is useful if you have already used it within windows and no longer have 
echo                the files saved to your windows setup folder.
echo.
...
echo              9. Start Windows updates For Win11 22H2-23H2 (Up to Aug 2024)
echo              10. Start Windows updates For Win11 24H2 (Up to Aug 2024)
echo.
...
IF /I '%INPUT%'=='9' GOTO 23h2
IF /I '%INPUT%'=='10' GOTO 24h2
...
:Morph2024
start .\sources\$OEM$\$$\Setup\FilesU\Apocalypse\Apocalypse_Install.cmd
start .\sources\$OEM$\$$\Setup\FilesU\Post_Installs_morphs_ultimate.cmd
GOTO Quit

:Slim
start .\sources\$OEM$\$$\Setup\FilesU\Apocalypse\Apocalypse_Install.cmd
start .\sources\$OEM$\$$\Setup\FilesU\Post_Installs_Slim.cmd
GOTO Quit

:Simp
start .\sources\$OEM$\$$\Setup\FilesU\Apocalypse\Apocalypse_Install.cmd
start .\sources\$OEM$\$$\Setup\FilesUPost_Installs_Simple.cmd
GOTO Quit

:Med
start .\sources\$OEM$\$$\Setup\FilesU\Apocalypse\Apocalypse_Install.cmd
start .\sources\$OEM$\$$\Setup\FilesU\Post_Installs_Min.cmd
GOTO Quit

:Full24
start .\sources\$OEM$\$$\Setup\FilesU\Apocalypse\Apocalypse_Install.cmd
start .\sources\$OEM$\$$\Setup\FilesU\Post_Installs_Full_2024.cmd
GOTO Quit
