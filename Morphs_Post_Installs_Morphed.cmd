@echo off
TITLE Morphs_Post_Installs_Morphed
chcp 65001 >nul
cd /d "%~dp0"
...
if exist "%~dp0Kaleidoscope_Grid_Morphed_Coloured.hta" (
    start "" "mshta.exe" "%~dp0Kaleidoscope_Grid_Morphed_Coloured.hta"
)
...
for /f "delims=" %%i in ('dir /b /a-d "%~dp0*.cmd"') do (
    ...
    if /i "!TargetScript!"=="Morphs_Post_Installs_Morphed.cmd" set "IsExcluded=1"
...
:: 120-second musical countdown sequence proceeds on the desktop
timeout /t 120
taskkill /f /im cmdmp3.exe >nul 2>&1
