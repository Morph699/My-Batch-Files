@echo off
cd /d "%~dp0"
TITLE Morphs Portables - Pack 1 (Below 100MB)
chcp 65001 >nul
...
:: 1. Dynamically count only .exe payloads UNDER 100MB
set "TotalInstalls=0"
for %%i in ("%~dp0*.exe") do (
    if %%~zi lss 104857600 set /a TotalInstalls+=1
)
...
:: 2. Process each small portable self-extractor sequentially
for %%i in ("%~dp0*.exe") do (
    if %%~zi lss 104857600 (
        ...
        echo ==============================================================================
        echo                  MORPHS PORTABLES - SMALL FILES [Below 100MB]
        echo ==============================================================================
        echo.
        echo      Installing Pack 1: !CurrentInstall! of !TotalInstalls!
...
:: TARGETED CLEANUP: Purges ONLY the processed .exe payloads under 100MB
if "!SimulationMode!"=="0" (
    for %%i in ("%~dp0*.exe") do (
        if %%~zi lss 104857600 del /f /q "%%~fi" >nul 2>&1
    )
)
